...
void Application::toGame()
{
    ...
    game_state = mixer_game_init_tries(channels, 0, std::vector<double> { -100.0, -12.0, -9.0, -6.0, -3.0 }, this);
    mixer_game_tries_add_audio_observer(game_state.get(), [this] (auto &&effect){ broadcastDSP(effect.dsp_states); });
    mixer_game_tries_add_audio_observer(game_state.get(), [this] (auto &&effect){ 
                                      channel_dsp_log(effect.dsp_states, channels); 
    });
    
    mixer_game_post_event_tries(game_state.get(), Event { .type = Event_Init }, nullptr);

    auto game_panel = std::make_unique<MixerGameUI_Tries>(
        channels,
        game_state->db_slider_values,
        game_state.get()
    );
    game_ui = game_panel.get();              
    mixer_game_post_event_tries(game_state.get(), Event { .type = Event_Create_UI }, game_ui);
...
void Application::onEditorDelete()
{
    ...
    {
        jassert(game_state);
        mixer_game_post_event_tries(game_state.get(), Event { .type = Event_Destroy_UI }, game_ui);
        game_ui = nullptr;
    }
}
...
void Application::initialiseEditorUI(EditorHost *new_editor)
{
    ...
    switch (type)
    {
        ...
        {
            jassert(game_state); 
            auto game_ui_temp = std::make_unique<MixerGameUI_Tries>(
                channels,
                game_state->db_slider_values,
                game_state.get()
            );
...
void Application::createChannel(int id)
{
    ...
    if (game_state) {
        ...
        mixer_game_post_event_tries(game_state.get(), event, game_ui);
    }
}
